...
public class SlagalicaGUI {
    ...
    private static final String ICON_PATH = "images/icon.ico";
    ...
    private void initialize() {
        ...
        frmSlagalica.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
        ...
        try {
            frmSlagalica.setIconImage(ImageIO.read(Thread.currentThread().getContextClassLoader().getResourceAsStream(ICON_PATH)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void updateTheList() {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    wordListModel.setValues(serbianDictionary.findAllWords(textField.getText()));
